...
function renderPlayers() {
  renderHome();
  if (!players.length) {
    playerList.innerHTML = '<p class="empty-msg">No players yet. Add one above.</p>';
    return;
  }
  playerList.innerHTML = players.map(p => `
    <div class="player-card" data-id="${p.id}">
      ...
      <button class="icon-btn del-player" data-id="${p.id}" title="Remove player">
...
function renderSessions() {
  renderHome();
  if (!sessions.length) {
    sessionList.innerHTML = '<p class="empty-msg">No sessions yet. Create one above.</p>';
    return;
  }
  sessionList.innerHTML = sessions.map(s => `
    <div class="session-card">
      <div class="session-header">
        ...
        <button class="icon-btn del-session" data-id="${s.id}" title="Delete session">
          <i class="bi bi-trash3"></i>
        </button>
      </div>
      ${s.address ? `<p class="session-addr"><i class="bi bi-geo-alt"></i> ${esc(s.address)}</p>` : ''}
...
// team assignment per player: 0 = none, 1 = team1, 2 = team2
let teamAssign = {};
...
function renderTeamGrid() {
  ...
  playersGrid.querySelectorAll('.player-tile').forEach(tile => {
    tile.addEventListener('click', () => {
      ...
      teamAssign[id] = ((teamAssign[id] ?? 0) + 1) % 3;
      renderTeamGrid();
    });
  });
...
function renderStats() {
  ...
  games.forEach(g => {
    ...
    allIds.forEach(id => { if (stats[id]) stats[id].played++; });
...
// ─── Boot ─────────────────────────────────────────────────────────────────────
// Pre-fill today's date in session form
